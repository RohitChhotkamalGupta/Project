	...
	if (document.getElementById("formname").value === "") {
		...
		formElements.forEach(element => {
			if (element.type === 'hiddentext') {
				formCode += `
        <input type="text" name="${element.label}",value="${element.label}" hidden><br>
      `;
			} else
				if (element.type === 'text') {
					formCode += `
        <input type="text" name="${element.label}"><br>
      `;
      ...
      `;
					...
					formCode += `
        <br>Question : <label for="${element.id}">${element.label}</label><br>
      `;
				}
				else if (element.type === 'radio') {
					formCode += `
        <input type="radio" id = "${element.label}" value="${element.label}" name="${element.name1}">
...
document.getElementById('generate-code').addEventListener('click', function() {
	var minutes = document.getElementsByName("timeInput")[0]; // Access the first element in the NodeList
...
<form id="dynamic-form" action="/ProjectBackup/ResponceController" method="post">
 <div id="form-container">
  <%String quizattempter = request.getParameter("quizattempter"); %>
	...
	<input type="text" id = "formname" name="formname" value="${formname.value}" hidden>
  ${formCode}
  ${scriptCode}<br>
